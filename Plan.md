...
Promotions (§8.7.4) remains fully deferred — the only §8.7 section with zero work done. `/admin/api/v1` (a separate token-authenticated admin API namespace per §10) was never built either — the admin panel talks to its own Inertia web controllers directly; out of scope for this pass. 2FA columns still unused pending a settings UI.*
...
- [ ] **In-app support chat:** user-side chat screen, Reverb delivery, push + email fallback, inbound email threading, canned replies — *partial (2026-07-16): backend built as part of §8.7.6 above — `GET/POST /api/v1/support/*`, Reverb WebSocket delivery + push + email on every staff reply, canned replies (admin-managed). Still pending: the React Native chat screen itself (mobile app, separate deliverable) and inbound email threading (a reply-by-email path, matching the pattern already used for team invites/webhooks, isn't wired for support — today the only way back in is the app or the admin panel).*
