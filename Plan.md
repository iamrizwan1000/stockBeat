...
| Back in stock — a product's stock goes from 0 to positive (added 2026-07-28) | poll |
...
**2026-07-28 addition — back in stock:** researched off a competitor feature ("preorder delivery fulfillment trigger") but deliberately scoped down to something honestly buildable. Fires once when a product's stock goes from 0 to a positive quantity, reusing the same `product_stock_snapshots` history as stale inventory — **WooCommerce-only today**, same reason. This is a **product-level** "this SKU is sellable again" signal, not a per-order "your specific preorder can now ship" trigger: `order_items` has no `product_id` link back to `products` (only a denormalized `sku` snapshot, and SKU isn't guaranteed unique or even present), so reliably resolving "which pending orders are waiting on this exact product" isn't buildable without a schema change this doesn't include. Sellers use the notification to go check on backordered/preorder line items themselves. No configurable threshold — the condition is binary (was it genuinely at zero before this?), so there's nothing to tune. Available Starter+ like every other trigger here (§5) — an operational fulfillment alert, not an anomaly-detection perk, same reasoning as every other trigger added this way.
...
### 4.12 AI Assistant — ask your business anything ⭐

- **Data Copilot:** a persistent "Ask AI" entry (plus a suggested-questions chip row on the Business Overview screen, and "Ask AI about this" on order detail / any notification) lets a seller ask free-form questions about their own store data — sales trends and period comparisons, top products, current inventory levels, unfulfilled/late orders, refunds, repeat buyers, reviews, why a specific alert fired, SMS/plan balance. Answers are produced by calling the app's own existing data Actions (§8.2/§8.3) as tools — never a raw database query and never a free-associated guess — so every number in an answer is real and traceable to a real endpoint. When a question falls outside what the app tracks today (profit/margin, ad spend, anything external to the app), the assistant says so explicitly rather than speculating.
- **Natural-language Rule Builder (Pro+):** "Notify me by text whenever an eBay order is over $200" is parsed into a real, structured rule (trigger + condition tree + action per §4.4/§8.4) and shown back for confirmation before it's saved — never auto-saved from a single ambiguous sentence.
- **Proactive AI Insights (Premium):** the assistant can also speak first — a new `ai_insight` rule trigger (§8.4) periodically scans a team's data for something notable (a channel dropping off, a connection needing reauth, an unusual refund pattern) and delivers it through the existing push/email notification pipeline, exactly like any other rule.
- **AI-narrated digest:** the existing daily/weekly digest (§4.6) can optionally render as a natural paragraph highlighting what's actually notable, instead of only the fixed "Yesterday: N orders, $X" template.
- **App Help — every plan, including Free:** how-to questions ("how do I connect Shopify"), plan/billing questions, and connection troubleshooting ("why isn't my store syncing," reasoning over real `store_connections.status`/`webhook_status`). Free because it deflects support tickets and helps activation (§4.11's funnel) rather than costing per-question against a business-data quota.
- **Voice input:** on-device speech-to-text feeds the same Ask endpoint as typed questions.
- **Multi-provider, admin-controlled:** answers are generated by whichever AI provider (OpenAI / Groq / Claude) is currently set active in the admin panel (§8.7.9) — a single global switch, changeable live with no app release, so an outage or cost change on one provider doesn't require a deploy to route around.
- **Guardrails:** every tool call is team-scoped (never cross-team data, same as every other endpoint); the data range the assistant can see mirrors the team's existing `plan_limits.analytics_level` — no separate depth concept to maintain; a monthly question quota is enforced server-side per plan (§5), with non-expiring top-up packs available exactly like SMS top-ups (§5).
...
² new order, high-value, unfulfilled-after-X, ship-by-deadline, order cancelled, refund requested, payment failed, low stock, negative review, positive review, stale inventory, back in stock, digest — the 2026-07-27/28 additions (positive review, stale inventory, back in stock) all get the same Starter+ "core trigger" treatment as low stock/negative review, not Premium, per the same "basic seller hygiene, not a luxury perk" reasoning already established for that group.
...
- **All numeric limits in this table (stores, rules, SMS allotment, history days, seats, trial days, advanced-trigger access) are NOT hardcoded** — they live in the `plans` / `plan_limits` database tables and are editable live from the admin panel (§8.7). Changing a limit takes effect on the next entitlement refresh, no app release required. Only the IAP *prices* and *product IDs* themselves must be changed in App Store Connect / Play Console (store-controlled) — `starter:monthly`/`pro:monthly`/`pro:yearly`/`premium:monthly`/`premium:yearly` (revised 2026-07-30 to Play Console's `{productId}:{basePlanId}` base-plan format) are RevenueCat's product identifiers, mapped to a plan tier in `ProcessRevenueCatEventAction::SUBSCRIPTION_PLAN_PRODUCTS`.
...
- AI Assistant — 150 questions/month, full history, plus a natural-language rule builder ("notify me when...")
...
- AI Assistant — 500 questions/month (highest of any plan), plus proactive AI insights that flag issues before you ask
...
**Web + mobile "what's in each plan" content, refreshed 2026-07-28**: the marketing home page (`welcome.tsx`) pricing cards were rewritten against the real `plan_limits` DB values (queried live, not just this section's prose) rather than the stale 5-bullet summary that predated most of §4.13-§4.24 — every bullet is now traceable to a real limit or capability flag, split into a shared "every plan includes" block (universal features like invoices/packing slips/order timeline/saved filters/bulk cost-price editing, none of which are tier-gated) plus real per-tier differentiators. Two inaccuracies were caught and fixed in the process: bulk cost-price editing isn't a Starter+ perk (no plan-limit gate at all, confirmed in code — it's universal), and "custom sound per rule" isn't Pro-exclusive (no plan-limit gate on the `sound` field either — any rule-creating tier already has it, so Starter has it too). A new mobile doc, `docs/mobile/plans-flow-screens.md`, carries the same verified content for an in-app "Compare plans" screen (which doesn't exist yet — today mobile has no plan-comparison screen at all, only `SubscriptionScreen` showing the seller's own current plan before handing off to RevenueCat's native paywall) — and flags that RevenueCat's own paywall copy (configured in its dashboard, outside this codebase) needs to be manually kept in sync with this list, since there's no automated link between the two.
...
- **Products (revised 2026-07-30 for Play Console's base-plan format):** `starter:monthly` ($5.99), `pro:monthly` ($17.99)/`pro:yearly` ($172.99), `premium:monthly` ($44.99)/`premium:yearly` ($429.99), consumables `sms_100` ($2.99), `sms_500` ($9.99), `ai_50` ($4.99), `ai_200` ($14.99). Mapped to a `Plan::key` in `ProcessRevenueCatEventAction::SUBSCRIPTION_PLAN_PRODUCTS` — built 2026-07-16, real end-to-end (webhook auth, idempotency via `revenuecat_events`, subscription state machine).
...
**Full-stack capability re-audit, 2026-07-28**: every `CapabilitySet::capabilities()` declaration across all 6 adapters was re-verified directly against code (not this table) — every `false`/declared-unsupported flag has a real, honest failure behind it (a clean `ValidationException`/`AdapterNotReadyException`, never a silent no-op or a 500), and every `true` flag has genuine implementation behind it. No new dishonest-capability bugs found this pass. This table is still missing a **TikTok Shop** column (built after this table was last touched) — TikTok is real-time orders ✅ (webhook + poll), fulfill/cancel ✅ (pre-shipment only), refunds ❌ (`refunds: false` — buyer-initiated only, no seller-refund API exists), messaging ❌ (`messaging_mode: 'none'`, not built), inventory update ❌, reviews ❌ — worth adding as a real column here later, not urgent since `docs/mobile/connections-api-reference.md`'s live `capabilities` response already covers it correctly. Three real mobile-doc bugs found and fixed in this pass: `rules-api-reference.md`'s `low_stock` platform-coverage row was stale (said "Shopify, WooCommerce only," omitting eBay's real `PollEbayInventoryJob` support that predates the doc's last edit); `connections-api-reference.md` never documented the `payouts_available`/`review_reply` capability fields despite both always being present in the real API response; and its `messaging_mode` enum was missing TikTok's real `"none"` value entirely.
...
#### 8.7.9 AI Assistant administration

- **Provider control:** one active-provider switch (OpenAI / Groq / Claude) plus a per-provider encrypted API key + model name (`ai_provider_settings`, §9) — same encrypted-credential discipline as `store_connections.credentials`, never exposed in any admin JSON payload. Changing the active provider or a key takes effect on the very next AI question, no deploy required — same live-effect guarantee `plan_limits` already gives every other setting. Every change writes to `admin_audit_log`.
- **Quota administration:** `ai_enabled` / `ai_questions_monthly` / `ai_rule_builder_enabled` / `ai_proactive_insights_enabled` are new `plan_limits` keys, editable exactly like every other limit in §8.7.3 — they slot into the existing Plans & Limits editor, no separate admin page needed.
- **Usage visibility:** per-team AI question usage (from `ai_usage_ledger`) surfaced on the Customer detail page (§8.7.2), alongside the existing SMS ledger — same page, same pattern.
...
subscriptions (id, team_id, provider: apple|google, rc_app_user_id,
               product_id: pro:monthly|pro:yearly, status: trial|active|grace|expired,
...
ai_provider_settings (id, provider: openai|groq|claude, api_key ENCRYPTED,
                       model, active, updated_by, updated_at)  // one row per
                       // provider; whichever has active=true is what
                       // AiProviderManager resolves at request time (§8.7.9)
ai_conversations (id, team_id, user_id, title, created_at)
ai_messages (id, conversation_id, role: user|assistant|tool, content,
             tool_calls JSON, created_at)
ai_usage_ledger (id, team_id, delta, reason: monthly_grant|topup_iap|question|freeze,
                 balance_after, meta)              // same shape as sms_ledger

-- AI Assistant Phase B (built 2026-07-22) --
-- products.cost_price (nullable, seller-entered only — no platform API
--   exposes true cost-of-goods) powers profit/margin questions.
-- orders.discount_amount / orders.tax (nullable) — real for WooCommerce
...
--   sales velocity rather than snapshot deltas (more accurate, simpler).
```
...
GET    /products · PUT /products/{id}/cost-price {cost_price}   → seller-entered cost basis (Phase B)
...
POST   /assistant/ask {question, conversation_id?, mode?: data|help}   → answer + tool calls used
                       // mode=data (default): the metered Data Copilot. mode=help: free/ungated
                       // App Help — no business-data tools offered, never debits the question quota.
GET    /assistant/conversations · GET /assistant/conversations/{id}
POST   /assistant/rule-draft {prompt}   → structured rule draft for confirmation
...
Admin namespace /admin/api/v1/* (admin-token + role middleware):
  ...
  /ai/providers (active provider + encrypted keys, CRUD) · /ai/usage (per-team quota view)
```
...
- [ ] **API-first:** `/api/v1` + `/admin/api/v1` namespaces, OpenAPI generation, Pest feature tests per endpoint (build & verify APIs before UIs) — *partial (2026-07-16): `/api/v1` namespace live (`routes/api/v1.php`), Pest feature tests per endpoint established as the working convention, unified `{success, message, data|errors}` response envelope (`App\Http\Responses\ApiResponse` + exception-handler rendering in `bootstrap/app.php`) applied to every endpoint. OpenAPI generation now done via `knuckleswtf/scribe` (`config/scribe.php`, scoped to `api/v1/*` only, ResponseCalls strategy removed in favor of hand-written `@response` examples on every controller method — none of these endpoints are safe to hit live without real team/auth state) — `php artisan scribe:generate` produces `docs/api/openapi.yaml` (+ Postman collection + static HTML), committed to the repo, all 34 `/api/v1` operations covered with bearer-auth security scheme, `@unauthenticated` correctly applied to the two OTP endpoints. `/admin/api/v1` still doesn't exist — the admin panel (`routes/web.php`) is Inertia + session-auth only, not a token-authenticated API; building that namespace (new auth guard/middleware, route file, and either duplicate or refactored controllers) is a separate, larger task, still pending.*
...
- [x] Redis + Horizon queues (`ingest`, `poll`, `rules`, `notify-*`, `actions`, `billing`) with per-store throttling middleware — *(2026-07-17): `QUEUE_CONNECTION=redis` (ddev-redis addon), Horizon installed with 7 named supervisors (`config/horizon.php`: ingest/poll/rules/notify/actions/billing/default) mapped to the queue names every job already targeted informally. `HorizonServiceProvider`'s gate restricts the `/horizon` dashboard to `admin` guard superadmins. Woo webhook ingestion moved off the request cycle: `WebhookController::woo()` now only verifies the signature and dispatches a new `ProcessWooWebhookJob` onto `ingest`, rather than mutating the DB inline. Per-store throttling via a new `ThrottlesPerStoreConnection` trait (`Illuminate\Queue\Middleware\WithoutOverlapping` keyed by connection id) applied to the Woo poll/webhook/rule-evaluation jobs, so two jobs for the same store never race each other. A real trait-property-collision bug surfaced and got fixed along the way: `RuleEvaluationJob`/`PollWoo*Job` had each declared `public $queue = '...'`, which fatal-errors against `Illuminate\Bus\Queueable`'s own `$queue` property — fixed by calling `$this->onQueue(...)` in the constructor instead of redeclaring the property.*
...
- [ ] Shopify adapter: OAuth, scopes, webhooks (+HMAC verify), GDPR webhooks, reconciliation poller — *update 2026-07-22: real Partner app credentials now configured (`SHOPIFY_CLIENT_ID`/`SHOPIFY_CLIENT_SECRET`), same for eBay (`EBAY_APP_ID`/`EBAY_CERT_ID`/`EBAY_RU_NAME`, sandbox), Etsy (`ETSY_KEYSTRING`), and TikTok Shop (`TIKTOK_SHOP_APP_KEY`/`APP_SECRET`) — `authorizationUrl()` now generates real, correctly-formed OAuth URLs for all four, live-verified. A real gap this surfaced and fixed in the same pass: `authorizationUrl()`/`completeConnection()` on Shopify/eBay/Etsy had no `assertConfigured()` guard (unlike Amazon/TikTok, which already had one) — they'd have silently built a broken URL with an empty `client_id` instead of failing cleanly with `AdapterNotReadyException` if a Partner/Developer app hadn't existed yet. All three now guarded the same way. Amazon remains the one platform genuinely still unconfigured (SP-API vetting, §15.2). Full end-to-end completion (a real merchant finishing the OAuth grant in a browser and the callback succeeding) is still unverified — that needs a real browser session against each platform's real consent screen, not something scriptable here.*
...
- [ ] **IAP billing:** RevenueCat SDK + products (`pro:monthly`, `pro:yearly`, `sms_100`, `sms_500`), `/hooks/revenuecat`, entitlement service, restore purchases, plan-gate middleware — *partial (2026-07-16): `POST /hooks/revenuecat` now real (`WebhookController::revenuecat`, outside `/api/v1` per the webhook-ingress pattern) — constant-time `Authorization` header compare against a self-generated `REVENUECAT_WEBHOOK_SECRET` (not something RevenueCat issues; you set the same value in its dashboard). `ProcessRevenueCatEventAction` is the state machine: `INITIAL_PURCHASE`/`RENEWAL`/`PRODUCT_CHANGE`/`UNCANCELLATION` → `subscriptions.status=active`; `BILLING_ISSUE` → `grace` (still treated as entitled by `Subscription::isEntitled()`); `EXPIRATION` → `expired`; `CANCELLATION` deliberately leaves status untouched (§6.1: it only means auto-renew was turned off — the entitlement doesn't actually lapse until `EXPIRATION`). `sms_100`/`sms_500` are handled separately as `NON_RENEWING_PURCHASE` consumables crediting `sms_ledger` (`SmsLedger::REASON_TOPUP_IAP`, which existed unused since the Notifications module — this is what it was for). New `revenuecat_events` table makes every event idempotent by RevenueCat's own event id before any side effect runs, since a redelivered webhook must never double-credit SMS. `app_user_id = user_id` resolves straight to our `User` → `currentTeam()`, matching §6.1's identity-linking design. Still pending: the RevenueCat SDK itself (React Native/mobile-side, nothing to build here until that project exists), restore-purchases (same), and a dedicated plan-gate middleware (entitlements are currently checked per-Action — e.g. `ConnectStoreAction`/`CreateRuleAction` — rather than centralized in middleware; works correctly today, a refactor not a gap).*
- [ ] **Admin panel (Inertia.js + React) per §8.7:** admin auth + 2FA + roles, KPI dashboard, customers module (list/detail/actions), plans & limits editor (DB-driven entitlements), promotions (offer-code campaigns + server comps), messaging center (segments, broadcasts, templates, delivery stats), **support inbox (live chat)**, ops/health boards, app config (min version, maintenance), audit log — *partial (2026-07-16): dedicated `admin` guard + `admin_users` table (roles: superadmin/support/readonly) via Fortify at `/admin/login`; Shopify Polaris UI with a real app-shell nav (`Frame`+`Navigation`+`TopBar`, matching genuine Shopify Admin — dark top bar with user-menu/sign-out, collapsible left sidebar with icons, replacing the earlier plain horizontal text-link bar) — Dashboard/Customers/Plans & Limits, each page `fullWidth` so content isn't squeezed into a narrow centered column. `admin_audit_log` built + every write action logs to it; `admin.write` middleware blocks the readonly role at the route level. **Dashboard (§8.7.1)**: signups, DAU/MAU, active trials + conversion %, paying subs (monthly/yearly split), MRR/ARR/ARPU/churn (IAP prices hardcoded from §5 pending a real prices table), top platforms, notification volume, SMS cost-vs-revenue, activation funnel — all computed live from real data; support-inbox metrics and "paywall seen" omitted (no `support_threads`/impression tracking). **Customers (§8.7.2)**: search/filter (plan/platform/signup/last-active) + CSV export + detail page (profile, devices, connected stores, subscription, rules, SMS ledger, notification volume, funnel position); country/LTV filters and support-chat history omitted (not tracked anywhere). **Customer actions**: extend trial, grant complimentary Pro, grant bonus SMS credits, force logout (revokes Sanctum tokens), suspend/unsuspend (added `users.suspended_at` + a `user.not_suspended` middleware that actually blocks the mobile API — verified live) — all audit-logged; GDPR export + account delete explicitly deferred. **Plans & Limits (§8.7.3)**: every `plan_limits` value live-editable, verified end-to-end that an admin edit reflects in the next mobile `/me` call with zero app changes; feature flags/paywall copy/SMS top-up pack editing deferred (no tables). Security fix along the way: `StoreConnection.credentials` was not in `$hidden` — fixed before building any admin view that touches store connections. **Messaging center (§8.7.5), built 2026-07-16**: `segments` (reusable saved audiences: plan/platform/inactive-days/trial-ending-soon/marketing-opt-in filters — `country` omitted, not tracked anywhere, same gap as Customers) + `ResolveSegmentAudienceAction` shared by the live preview-count endpoint, real sends, and single-user announcement matching. `broadcasts` (compose to all/a segment/a single user, push+email+banner channels, `{first_name}`/`{plan}`/`{trial_days_left}` template variables) + a real per-recipient-per-channel `broadcast_deliveries` log — deliberately not a mutable JSON stats counter, since sends happen via queued `SendBroadcastToRecipientJob`s and a shared counter would race; delivery *attempts* (sent/failed/skipped-no-consent/skipped-muted/skipped-quiet-hours) are real, "delivered"/"opened" are not tracked (no receipt/open-pixel infra) and never fabricated. Guardrail ("superadmin approval required for all-users sends") enforced as a role check — at creation time for a *scheduled* all-audience send (so a cron dispatch, `messaging:send-scheduled-broadcasts` every 5 min, can never get stuck unattended) and at send time for an immediate one; the sending superadmin is recorded as `approved_by`. Broadcast emails always gate on `marketing_opt_in` (marketing, not transactional); banner channel always logs an in-app `Notification` regardless of consent, matching how push already treats the in-app center as "a record of what fired," not a personal-preference-gated channel. "Send test to self" renders variables against fixed placeholder values (admins have no team/subscription to interpolate against). **Announcements**, also built as part of this pass since §10's `GET /config` needs them: CRUD + a new real `GET /api/v1/announcements` (audience-matched, active-window-filtered) — scoped to just announcements, not a full `/config` (`min_version`/remote flags aren't built). 25 new Pest tests; full suite (270), Pint, and PHPStan all clean.

**Operations & health (§8.7.7), built 2026-07-16**: a real `app_config` key-value table (matches §9 exactly) + admin-editable `min_version`/`maintenance_mode`/`maintenance_banner` — no mobile-facing `GET /config` reads these yet (same scope cut as announcements above; the admin side is real, the mobile-consumption side is a separate, still-pending piece). `GetOpsHealthSnapshotAction` is a live snapshot, deliberately not a synthetic history/trend chart — nothing logs webhook-failure or poller-lag *history* yet, only current state: connection health (needs_reauth/disconnected/stale >2h/never-synced counts, real query over `store_connections`), failed jobs (real, from Laravel's own `failed_jobs` table — nothing custom), SMS spend this month + top-consuming teams (real `sms_ledger` aggregation), notification volume by type over the last 24h, and an abuse guard flagging any team firing over 50 rule executions/hour (arbitrary threshold, documented in code). One spec item honestly still not built: Etsy/Amazon API quota usage (those adapters are still stubs — nothing to meter).

**Trial-abuse fingerprinting (§8.7.7 gap), built 2026-07-17**: two new real signals feed `GetOpsHealthSnapshotAction`'s `abuse` block. First, `store_connections.fingerprint` — a `sha256(platform|normalized_store_url)` computed by `ComputeStoreConnectionFingerprintAction` and persisted by `ConnectStoreAction` on every successful connect — flags the same underlying shop reconnected under more than one team (classic multi-trial farming). Only WooCommerce carries a real store identity today (`credentials.store_url`); Shopify/eBay/Etsy/Amazon adapters are still stubs that throw before a connection is ever created, so there's nothing to fingerprint for them yet — documented in the action's own docblock rather than silently no-op'd. Second, `users.signup_ip` — captured by `VerifyOtpAction` only on first account creation, never overwritten on a repeat login — flags a signup IP shared by more than one team that has actually consumed a trial (has a subscription row), surfaced via a new `DetectTrialAbuseFlagsAction`. Both signals are explicitly documented as honest best-effort signals, not proof (a shared office/VPN IP or a legitimate store reconnect after an account handoff look identical here) — flagged for admin review on the Ops & Health page, never auto-actioned. SMS anomaly detection from the same spec bullet is still not built.

**Support inbox (§8.7.6), built 2026-07-16**: real-time delivery via Laravel Reverb (`composer require laravel/reverb`, `BROADCAST_CONNECTION=reverb`) — the first broadcasting use in the codebase, so `bootstrap/app.php` now registers `/broadcasting/auth` with `auth:admin,sanctum` (tries both guards) since mobile users (Sanctum) and admin staff (session guard `admin`) both need to subscribe to the same private `support-thread.{id}` channel; `routes/channels.php` authorizes staff for any thread and end users only for their own. `support_threads`/`support_messages`/`canned_replies` tables match §9. One thread per user, resumed rather than recreated; a user's new message reopens a resolved thread. Staff replies deliver three ways at once — WebSocket broadcast, push, and email — rather than the spec's presence-based "push+email only when not in-app," since no presence-detection mechanism exists to know that; each delivery attempt is recorded on the message (`delivered_via`). Internal notes (`direction=note`) are stored but never broadcast to the user and never returned by the mobile endpoint. Admin side: thread list (status/unassigned filters), thread detail (message timeline, reply composer with canned-reply quick-insert, internal notes, assign-to-me/unassign, resolve), separate Canned Replies CRUD page. Mobile: `GET /api/v1/support/thread` + `POST /api/v1/support/messages`. Real, honest gap: no live Reverb server process was started in this dev environment to manually verify an actual socket connection end-to-end — broadcasting is wired and event-tested (`Event::fake()`/`assertDispatched`), same category of cut as "didn't stand up a real WordPress+WooCommerce instance" elsewhere in this log. CSAT (👍/👎 after resolve) and auto-close-after-7-days-idle from §4.9 are not built. 22 new Pest tests across both modules; full suite (292), Pint, and PHPStan all clean.

**Inbound email threading for support chat, built 2026-07-17**: the missing "reply by email" half of §4.9/§8.7.6. A shared plus-addressed inbound-email pattern (`support+{thread_id}@{domain}`, `thread+{id}@{domain}` — the latter reused by the unified customer inbox below) is parsed by a new `ParseInboundEmailTokenAction` and routed through a single webhook, `POST /hooks/email-inbound` (`WebhookController::emailInbound`), authenticated by a shared-secret bearer token — the same pattern already used for the RevenueCat webhook, since no real inbound-email provider (SES/Mailgun/Postmark) is actually connected in this environment; wiring the same endpoint to a real provider later is a config change, not a rewrite. `ReceiveInboundEmailReplyAction` appends the reply to the matching `support_thread` and reopens it if resolved — but only when the `from` address matches the thread's own user; a mismatch is silently dropped, since anyone who learned a `support+{id}@` address could otherwise inject a fake reply into another user's thread. Honest scope cut, documented in code: no real MIME parsing (attachments, multi-part bodies) — only a plain-text `text` field from the inbound payload is handled, matching what an actual provider's webhook would need real credentials to test against anyway.

**Promotions & discounts (§8.7.4), built 2026-07-17**: `promo_campaigns` table (`type: offer_code|intro_offer|server_comp`, `store_ref`, `config` JSON, `starts_at`/`ends_at`, `stats` JSON) + full CRUD admin page. `offer_code`/`intro_offer` are honestly metadata-only — this app never talks to the Apple/Google store consoles, and there's no verified RevenueCat payload field tying an event back to a specific offer-code campaign, so redemption/conversion stats for those two types stay whatever an admin enters by hand rather than a fabricated automatic count. `server_comp` is the one type this app actually executes: `ApplyServerCompToSegmentAction` reuses the existing `ResolveSegmentAudienceAction` (from Messaging, §8.7.5) to resolve a segment (or "everyone," superadmin-gated the same way an all-users broadcast is) into teams, then reuses the existing `GrantComplimentaryProAction`/`GrantBonusSmsCreditsAction` (from Customers, §8.7.2) to apply the comp to every team — `stats.applications` records each real application (segment, recipient count, timestamp), never a synthetic number. Verified live in-browser (Playwright): campaign creation, type-switching the form, and the "Apply" flow against a real segment.

**Admin team & audit (§8.7.8), built 2026-07-17**: `admin_audit_log` already existed and every write action already logged to it (per the original admin-panel pass) — what was missing was a way to *see* it. New searchable/filterable Audit Log page (`ListAdminAuditLogAction`: filter by admin/action-substring/target-type/date-range, paginated) plus a new Admin Team page: create admin users (superadmin-only — there's no self-registration or email-invite flow for the admin guard, per `config/fortify.php`'s own note, so a superadmin sets the initial password directly), change another admin's role, remove an admin, and a "reset 2FA" action clearing `two_factor_secret`/`two_factor_recovery_codes`/`two_factor_confirmed_at`. Every one of these guards against acting on your own account (no self-role-change, no self-delete, no self-2FA-reset) to prevent an accidental lockout. Real bug found and fixed here: `AdminUser`'s `#[Fillable(...)]` attribute doesn't include the `two_factor_*` columns, so a plain `->update([...])` on them silently no-ops — fixed with `->forceFill([...])->save()`. Honest gap, unchanged from before: Fortify's `twoFactorAuthentication` feature is still intentionally disabled on the admin guard pending a self-service 2FA *setup* UI, so `two_factor_*` columns are never actually populated today — this reset action exists as the admin-side "clear a stuck state" capability for the moment that UI ships, not a currently-exercised one.

**Customers — country/LTV filters + support-thread history (§8.7.2 gaps), built 2026-07-22**: `country` is real now — `orders.shipping_country` is a new plaintext column populated at ingest (`IngestOrderAction`) specifically because `orders.shipping_address` is encrypted at the column level and can never be filtered in SQL at all; the filter means "has shipped at least one order to this country," not a single fixed attribute (a seller's customers aren't all in one place). `ltv_min`/`ltv_max` also real — `ComputeCustomerLtvAction` already existed for the detail page, but LTV's per-event FX conversion isn't a portable SQL aggregate, so when either bound is set, `ListCustomersAction` loads the SQL-filtered candidates, computes LTV in PHP, and paginates the filtered collection manually; fine at this app's real admin-tool scale, would need a cached `teams.ltv_cached` column if the customer base ever got large enough for that to matter. LTV also now shown as a column on the list and in the CSV export, not just the detail page. Support-chat history is a real read of `support_threads`/`support_messages` on the Customer detail page (status, priority, last 5 messages, link to the full thread) — the standalone Support Inbox page (§8.7.6) stays the place to actually reply; this is a summary, not a duplicated data source. 6 new Pest tests.

**Dashboard — feature adoption (§8.7.1 gap), built 2026-07-27**: closes the gap left by the §4.13-§4.23 modules shipping with zero admin-side visibility — until now there was no way to know whether any of them were actually being used. New `feature_usage_events` table + `FeatureUsageEvent::log($team, $feature)`, called from every action with no other natural signal of use: invoice generation, single and bulk packing-slip generation, bulk-cancel, bulk-tag (one row per API call, not per order, for the bulk actions). `reviews.replied_at` is a new real column too — replying to a review was previously entirely stateless, so reply rate had nothing to compute from. `ComputeFeatureAdoptionAction` (a sibling to `ComputeDashboardKpisAction`, not folded into it, since it's a distinct concern) aggregates all of this plus rule adoption (positive_review/stale_inventory trigger counts, monthly-digest count, priority breakdown) and — genuinely new, enabled by the order-timeline work — quick actions taken, grouped by type straight from `order_events`. `digest_frequency` is filtered in PHP rather than a JSON-path SQL `where()`, matching the same "stay portable across MariaDB/SQLite" discipline used elsewhere (e.g. `whereDate()` over `DATE_FORMAT()`), and digest rules are never numerous enough for that to cost anything. New "Feature adoption" section on the Dashboard page, reusing the existing `Stat`/`SectionCard` components.

**Dashboard — paywall hits (§8.7.1 gap), built 2026-07-27**: the upsell-conversion counterpart to feature adoption above — which real plan-limit gates teams are actually running into. New `paywall_hits` table (`team_id`, `limit_key` reusing `PlanLimit`'s own constants rather than duplicating them, `plan_key` snapshotted at hit time via `Subscription::effectivePlanKey()`, `occurred_at`) + `PaywallHit::log($team, $limitKey)`, wired into every real gate-check rejection found by grep across the codebase: bulk actions, payouts/reviews/inbox (`analytics_level`/`inbox_enabled`), store connections (`max_stores`, both `ConnectStoreAction` and `StartOAuthConnectionAction`), rule creation/editing (`max_rules`, `advanced_triggers_enabled`, `ai_proactive_insights_enabled`), the AI rule builder (`ai_rule_builder_enabled`), team invites (`team_seats`), analytics history range (`analytics_level`), and the AI Data Copilot (`ai_enabled` + the separate `ai_questions_monthly` quota signal) — 12 call sites across 10 files, deliberately every *server-side rejection*, not a client-side upsell-modal impression (that still isn't tracked, nothing logs a paywall screen being shown, only a request actually being blocked). `ComputePaywallHitsSnapshotAction` aggregates total/last-30-days, a by-gate breakdown, and a top-10 "teams to reach out to" list (most hits in the last 30 days) — the clearest upgrade-outreach signal this dashboard can produce, since a repeat hit means the feature is wanted, not just glanced at. New "Paywall hits" SectionCard on the Dashboard, placed just above the existing Activation funnel, whose caption is updated to stop claiming "paywall seen isn't tracked" in a way that could now read as stale — it still can't insert a literal funnel step (teams hit different gates at different points), but the new section directly answers what that caption used to flag as missing.

`/admin/api/v1` (a separate token-authenticated admin API namespace per §10) still was never built — the admin panel talks to its own Inertia web controllers directly; no consumer needs it today (no third-party integration, no separate admin client), so building a parallel API surface with nothing to call it would be premature. Revisit if that changes.*

Also fixed in this pass: every admin sidebar nav link (not just the three new ones) was silently broken — `AdminLayout`'s `linkComponent` bridge read a `href` prop, but Polaris's `LinkLikeComponent` contract passes the URL as `url`, so every `<a>` rendered with `href=""` and went nowhere on click. And added a public marketing landing page at `/` (previously a bare redirect straight to `/admin`) — Tailwind, not Polaris (Polaris per §8.7 is admin-only), pulling real copy from §1/§4/§5 rather than placeholder text.
- [ ] **In-app support chat:** user-side chat screen, Reverb delivery, push + email fallback, inbound email threading, canned replies — *partial (2026-07-17): backend built as part of §8.7.6 above — `GET/POST /api/v1/support/*`, Reverb WebSocket delivery + push + email on every staff reply, canned replies (admin-managed). Inbound email threading now built (`POST /hooks/email-inbound`, see §15.3 admin-panel notes above) — a user can reply to the email notification and have it thread back into the same support conversation. Still pending: the React Native chat screen itself (a separate, mobile-app deliverable).*
...
- [ ] **Trial system:** 7-day server-side trial, countdown UX, day-5/day-7 notifications, trial-end downgrade flow exactly per §6.4 (pause stores, freeze rules/credits, truncate display history) — *partial (2026-07-17): `GrantTrialSubscriptionAction` grants the trial on team creation (`subscriptions.status=trial`, `trial_ends_at` = Pro's admin-editable `trial_days` limit, default 7). **Day-5/day-7 reminders, built 2026-07-17**: `trials:send-reminders` (scheduled hourly) computes `daysRemaining = floor(hoursRemaining / 24)` — deliberately `floor()`, not `ceil()`, since an earlier draft's `ceil()` rounded "2 hours left" up into the wrong reminder bucket, a real bug caught before shipping — and sends `TrialEndingMail` once per team per bucket (`day5_reminder_sent_at`/`day7_reminder_sent_at` timestamps prevent a re-send on the next hourly run). `subscriptions:expire-trials` (scheduled daily) flips any trial past `trial_ends_at` to expired. **§6.4 downgrade freeze, built 2026-07-17**: `ApplyDowngradeFreezeAction`/`ReverseDowngradeFreezeAction` — freezes are reversible-by-design ("freezes, never destroys," per §6.4), implemented as tracking columns rather than deletion: `store_connections.paused_at` (stores beyond the new plan's `max_stores` limit, oldest-first), `rules.auto_disabled_at` (rules beyond `max_rules`), `team_members.suspended_at` (seats beyond `team_seats`, enforced by a new `team.not_suspended` middleware that actually blocks the mobile API). `ReverseDowngradeFreezeAction` restores "up to the new plan's limits, oldest-first" on any re-upgrade — wired into the RevenueCat webhook, the admin "grant complimentary Pro" action, and trial extension, so a comp or an organic purchase both correctly spring a frozen team back the same way. Display-history truncation isn't a separate mechanism — the existing plan-gated `history_days` limit on the order feed already covers it. Countdown UX is a mobile-app-only concern, nothing left to build server-side beyond what `GET /me`'s subscription payload already exposes. **Real bug fixed 2026-07-28**: `ExtendTrialAction` (the admin Customer Detail "extend trial" action) never actually set `plan_key` — a team with no subscription row yet got a fresh one with `plan_key: null` (resolving to Free entitlements despite `status: trial`), and reviving a lapsed *paid* subscription (e.g. an expired Starter/Pro team) left the old, lower `plan_key` in place, silently granting only that tier's limits instead of the "full-featured trial taken literally" every other trial path guarantees. Now sets `plan_key: premium` unconditionally, matching `GrantTrialSubscriptionAction`'s own behavior — 2 new Pest tests covering both cases.*
...
- [ ] Unified inbox: eBay + Etsy messaging, SES email threading for Shopify/Woo, reply templates, assignment — *partial (2026-07-17): the Shopify/Woo order-linked email-threading half built ahead of schedule (Phase 2 item pulled forward alongside the other backend modules this pass). New `inbox_threads`/`inbox_messages`/`reply_templates` tables — `GetOrCreateInboxThreadAction` resumes the same thread on repeat contact about the same order rather than recreating it (`firstOrCreate` keyed on `order_id`). Outbound messages (`SendInboxMessageAction`) send from the app's own domain with a plus-addressed `Reply-To` (`thread+{id}@{domain}`) rather than "the merchant's connected email" — the spec offers that as an alternative; this implements the other, simpler half. Inbound replies land through the same shared `POST /hooks/email-inbound` webhook built for support chat above (`ReceiveInboxReplyAction`), with the identical from-address-must-match-the-thread security check. Reply templates support `{customer_name}`/`{order_number}`/`{tracking}` substitution (`RenderReplyTemplateAction`) and can be applied directly when sending a thread message. Order fulfillment (`FulfillOrderAction`) now persists `tracking_number`/`carrier` onto the order so the template variable has something real to substitute. A real PHPStan quirk was hit and worked around here: `$order?->order_number ?? ''` was flagged `nullsafe.neverNull` (a known false-positive interaction between PHPStan's bleeding-edge coalesce rule and Larastan's relation-property inference, reproduced in isolation) even though `$thread->order` is genuinely nullable — resolved by assigning to a local variable first and using an explicit `!== null` ternary instead of the nullsafe-plus-coalesce chain. eBay + Etsy native messaging remain blocked on those adapters, which are still stubs.*
...
- [x] **AI Assistant — Data Copilot + App Help (§4.12), built 2026-07-22:** `POST /assistant/ask` tool-calling agent over existing Actions (`GetAnalyticsSummaryAction`/`GetTopProductsAction`/`ListOrdersAction`/low-stock/connection-health/account-status) — team-scoped, never raw SQL. Two explicit, separately-gated modes rather than inferred intent (`AskAssistantAction::MODE_DATA`/`MODE_HELP`): the Data Copilot requires `plan_limits.ai_enabled` and debits `ai_usage_ledger`'s monthly quota (mirrors `sms_ledger`, quota computed by counting this calendar month's `question`-reason rows rather than a separate grant job); App Help (`get_connection_health`/`get_account_status` tools only) is reachable on every plan including Free and never touches the quota — this was a real bug on the first pass (the whole endpoint was gated by `ai_enabled`, silently locking Free out of App Help entirely, contradicting this exact spec line) caught and fixed. `ai_conversations`/`ai_messages` persist full history including tool round-trips as a genuine audit trail. 11 Pest tests.
- [x] AI Assistant — multi-provider admin control (§8.7.9), built 2026-07-22: `ai_provider_settings` (OpenAI/Groq/Claude — `OpenAiProvider`/`GroqProvider` share one OpenAI-compatible implementation, `ClaudeProvider` does real message-shape translation for Anthropic's distinct Messages API/tool format), encrypted `api_key` + `#[Hidden]`, single global active-provider switch via `AiProviderManager` (`Illuminate\Support\Manager`, same pattern as `ChannelAdapterManager`) — live effect with no deploy, audit-logged. Real Polaris admin page (`/admin/ai-assistant`) alongside the artisan bootstrap path (`ai:set-provider`); new `ai_*` `plan_limits` keys wired into the existing Plans & Limits editor (labels + boolean coercion). 5 Pest tests.
- [x] AI Assistant — natural-language Rule Builder (Pro+), built 2026-07-22: `POST /assistant/rule-draft` (`GenerateRuleFromPromptAction`) — single-shot structured generation (no tools), validated through the exact same `StoreRuleRequest` rules `POST /rules` uses, never auto-persists.
- [x] AI Assistant — AI-narrated digest, built 2026-07-22: `NarrateDigestAction` rephrases `SendMorningDigestAction`'s already-computed real numbers into a natural sentence on `ai_enabled` plans (Starter+; Free keeps the template — narrating every Free team's digest daily would cost real LLM money with no revenue behind it); resilient by design, any provider failure returns `null` and the template body is used instead, so a digest can never fail to send because narration did. 3 Pest tests (the full `SendMorningDigestAction` integration path can't be verified locally — see the pre-existing Firebase-credential gap noted elsewhere in this log — but `NarrateDigestAction` itself is fully tested in isolation).
- [x] AI Assistant — Proactive AI Insights (Premium), built 2026-07-22: new `Rule::TRIGGER_AI_INSIGHT`, gated at creation/update (`CreateRuleAction`/`UpdateRuleAction`) by a dedicated `ai_proactive_insights_enabled` limit, separate from `advancedTriggers()`. `DetectAiInsightsAction` deliberately does NOT let the AI decide what's notable — it reuses the same deterministic-threshold discipline `order_spike`/`refund_spike`/`low_stock` already use (today-vs-yesterday revenue drop ≥25% via `GetAnalyticsSummaryAction`, any `needs_reauth`/`disconnected` connection, low-stock count) and only calls `NarrateInsightAction` to phrase real, already-detected facts. Fires through the ordinary `RuleEvaluationAction`/`DispatchRuleActionsAction` path (order-less, so `controls.cooldown_minutes` is what throttles repeats, same as `digest`/`low_stock`). New hourly `ai:detect-insights` command, plan-checked live on every run (not cached at rule-creation time) since a downgrade/comp/expiry can change a team's plan without touching its rules. 6 Pest tests.
- [x] AI Assistant Phase B, built 2026-07-22: `cost_price` on `products` (nullable, seller-entered only — `PUT /api/v1/products/{product}/cost-price`, `GET /api/v1/products`; no platform API exposes true cost-of-goods, so `null` clears rather than fabricating zero-cost); `discount_amount`/`tax` on `orders`, mapped for real from WooCommerce's own `discount_total`/`total_tax` fields (`WooOrderMapper`) — every other platform stays `null` since their adapters can't connect yet (§15.3), never fabricated; `product_stock_snapshots` (append-only, written by `PollWooProductsJob` alongside every managed-stock update). Two new Data Copilot tools: `get_profit_summary` (revenue − cost, but *only* for order items whose product has a real cost price — excluded units are reported explicitly, never assumed zero-cost) and `get_restock_recommendations` (deliberately uses real sales velocity from `order_items` over the last 14 days rather than the new snapshot table's deltas — more accurate and simpler; the snapshot table itself is real, populated infrastructure, just not this particular tool's data source). `AskAssistantAction`'s system prompt updated so profit/restock questions are no longer listed as unanswerable — with an explicit instruction to state what was excluded. 23 new Pest tests. **Live-verified against the real Groq API** with real order/cost/stock data — profit math confirmed exactly correct ($45 revenue − $12 cost = $33 profit on 3 units), restock estimate confirmed exactly correct (6 units/14 days → 14-day stockout estimate). One honest, real finding from that live testing: Groq's fast model occasionally fails to produce a valid tool call on a *compound* two-part question ("what's my profit AND should I restock") — single-intent questions were reliable across repeated attempts; the app already handles this gracefully (a clean `AiProviderException` → 502, no quota debited, no partial state) rather than crashing, so this is a known real-provider characteristic to note, not a code defect.

**Honest gap, unchanged from the rest of this log's own discipline:** none of the above has been live-verified against a real OpenAI/Groq/Claude API call — every test uses `Http::fake()`. Also never run against the real ddev/MariaDB dev environment (Docker wasn't available this session) — only local PHP + SQLite. Both are the same category of gap already documented elsewhere in this checklist (e.g. "didn't stand up a real WordPress+WooCommerce instance"), not a correctness question, but a real one to close before calling this done.

### 15.5 Phase 3

- [ ] Amazon adapter (SQS notifications, RDT-gated PII, feeds for tracking, template messaging)
- [x] Spike/anomaly triggers, goal tracking, multi-currency consolidation — *(2026-07-17): spike triggers (`order_spike`/`refund_spike`) and goal tracking (Analytics-lite, §15.3) both built ahead of schedule. **Multi-currency consolidation, built 2026-07-17**: new `fx_rates` table (date, base, target, rate) populated by `SyncFxRatesAction`/`fx:sync-rates` (scheduled daily) against the Frankfurter API (`https://api.frankfurter.dev/v1/latest`) — free, no API key, real ECB-derived rates, not a fabricated/hardcoded conversion table. `ConvertToBaseCurrencyAction` looks up same-day (or most recent prior) rate and converts; `IngestOrderAction` now calls this instead of only handling the exact-match case, so `total_base_currency` is correctly populated on genuine cross-currency orders instead of staying `null`. `orders:backfill-base-currency` command re-processes existing orders once rates exist. Real cross-database bug found and fixed here: `FxRate`'s `date` cast round-trips through a full `Y-m-d H:i:s` string on save, which MariaDB silently truncates to just the date but SQLite does not — a plain `where('date', ...)` comparison worked in dev (MariaDB) but silently failed in every test (SQLite); fixed by using `whereDate()` everywhere this column is compared, in both production code and test assertions.*
...
- [x] Trial/downgrade end-to-end test (trial grant → expiry → freeze → re-upgrade restore) — *(2026-07-17): `TrialLifecycleTest`/`DowngradeFreezeTest` cover the chain in linked pairs rather than one single mega-test — `'a lapsed trial is expired and frozen exactly once'` chains expiry straight into the freeze (stores paused oldest-first, rules auto-disabled, seats suspended), and `'unfreezing restores stores, rules, and members up to the new plan limits, oldest first'` chains a re-upgrade straight into the restore, verified through `ReverseDowngradeFreezeAction`'s real call sites (RevenueCat webhook, admin comp grant, trial extension) rather than invoked directly. Trial *grant* itself (`GrantTrialSubscriptionAction`) was already covered from the earlier team-creation work — not re-tested here, just relied on.*
