...
`entitlements.limits.ai_questions_monthly` (`auth-api-reference.md`) is the plan's monthly cap — `null` means unlimited. **There's no `GET` endpoint exposing "questions used so far this month" separately** — the only way to know where you stand is the 422 itself when the cap is hit, or by counting successful `mode: "data"` asks client-side for the current calendar month as a soft local estimate (not authoritative — a multi-device team shares one quota, so another member's usage won't be reflected locally). Consider showing a low-remaining warning only after you've actually hit the 422 once, rather than trying to track a running counter that can't be kept in sync with other devices.

**There is no AI question top-up pack today** (unlike SMS's `sms_topup_packs`, `settings-api-reference.md`) — the backend has the ledger reason code reserved for it, but no purchasable product exists yet. Don't build a "buy more AI questions" purchase flow; when the quota is hit, the only real options are waiting for next month's reset or upgrading the plan.
