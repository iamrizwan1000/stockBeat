...
6. **Payouts** → `PayoutsScreen`, fully documented in `payouts-flow-screens.md` (added 2026-07-26). **Pro+ only** (`entitlements.limits.analytics_level === "full"`) — gate like the Team & Roles row above, upgrade prompt for Free/Starter. Read-only reconciliation view; only shows real data for connected Shopify stores.
7. **Reviews** → `ReviewsScreen`, fully documented in `reviews-flow-screens.md` (added 2026-07-26). **Pro+ only** (`entitlements.limits.inbox_enabled`) — same gate as the Inbox tab, since replying to a review is the same underlying capability. Reply control additionally only shows for eBay-sourced reviews within this already-gated screen.
...
**Upgrade / change plan:** show a "Compare plans" screen first — see `plans-flow-screens.md` (added 2026-07-28) for the verified, DB-checked feature list per tier — then open the RevenueCat SDK's native paywall/purchase sheet directly to actually purchase. Do not build a custom pricing table calling any endpoint on this API for the purchase itself, there isn't one for subscription products; the compare screen is informational copy only. After a purchase completes in the SDK, **poll `GET /me` for up to ~10–15s** (a few retries) rather than assuming the immediate next call already reflects the new plan — the webhook that actually updates entitlements is asynchronous server-side.
