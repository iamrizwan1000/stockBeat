...
Added 2026-07-30, prompted by a real audit that found (and fixed) several genuine double-submission bugs — not just documentation gaps. The behaviors below reflect the real, current, fixed state — see each endpoint's own reference doc for anything not covered here.
