...
**Important correction vs. what you might see in the auto-generated OpenAPI docs (`docs/api/openapi.yaml`):** those show a `200` response for the "not supported" scenario on fulfill/refund/cancel — that annotation is stale. The real, verified behavior (confirmed by calling the action directly) is **422** for every "not supported"/failure scenario on all three, **with the message under `errors.order[0]` for the capability-based "not supported" cases specifically** — same as any other validation failure. Fulfill's multi-location case is the one exception (top-level `message` instead, see above) — check both. Either way, handle it as a normal 422, not a `200` with `success: false`.
