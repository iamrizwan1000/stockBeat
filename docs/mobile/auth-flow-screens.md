...
| `sells_on` | multi-select chips | ✅, min 1 | Exactly these six values — **note the value is `woo`, not `woocommerce`**: `shopify` `woo` `ebay` `etsy` `amazon` `tiktok`. Display labels can say "Shopify," "WooCommerce," "TikTok Shop," etc. — just send the raw key. **Updated 2026-07-31 — put a small "Coming soon" badge on the five non-Shopify chips, for honesty/consistency with the connect-picker (`connections-flow-screens.md`) and the marketing site, both of which now show the same badge.** Unlike those two, **keep all six chips selectable here** — this is a survey question ("what do you sell on"), not an attempted connection, so there's no 422/dead-end risk to guard against by disabling them, and disabling would throw away genuinely useful signal (real demand for eBay/Etsy/etc. before they launch). Don't copy the connect-picker's non-interactive treatment onto this screen. |
